...
const ABCs: [char; 26] = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'];

fn priority(letter: &char) -> u32 {
    let is_lowercase = letter.is_lowercase();
    let position = ABCs.iter().position(|&x| x == letter.to_ascii_lowercase()).unwrap();
    let priority = if is_lowercase { position + 1 } else { 27 + position };
